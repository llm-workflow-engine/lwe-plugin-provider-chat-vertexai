...
with open(path.join(FILE_DIR, 'lwe_plugin_provider_chat_vertexai', 'version.py')) as f:
    version = re.match(r'^__version__ = "([\w\.]+)"$', f.read().strip())[1]

setup(
    name="lwe-plugin-provider-chat-vertexai",
    version=version,
    author="Chad Phillips",
    author_email="chad@apartmentlines.com",
    description="LWE plugin: Chat VertexAI Provider plugin",
    ...
    entry_points={
        "lwe_plugins": [
            "lwe_plugin_provider_chat_vertexai = lwe_plugin_provider_chat_vertexai.plugin:ProviderChatVertexai"
        ]
    },
)
